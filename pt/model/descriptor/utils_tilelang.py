...
@tilelang.jit
def fused_angle_update_backward_inputs_v3(
    M,
    K,
    A,
    N,
    ...
    """V2 with coalesced float4 producers and XOR-swizzled [row, K] tiles."""
    assert dtype == "float32"
    assert BLOCK_M == 32
    assert BLOCK_D == 32
    assert BLOCK_K == 32
    ...
    def angle_backward_inputs(
        ...
        with T.Kernel(T.ceildiv(M, BLOCK_M), T.ceildiv(D, BLOCK_D), threads=THREADS) as (bx, by):
            grad_shared = T.alloc_shared((BLOCK_M, BLOCK_K), dtype)
            # Keep weights logically row-major [D, K]. GEMM consumes B.T.
            weight_shared = T.alloc_shared((BLOCK_D, BLOCK_K), dtype)
            acc = T.alloc_fragment((BLOCK_M, BLOCK_D), accum_dtype)
            bias_tile = T.alloc_fragment((BLOCK_K,), accum_dtype)
            T.annotate_layout({
            ...
            for ko in T.Pipelined(T.ceildiv(K, BLOCK_K), num_stages=2):
                # 32 rows * 8 float4 groups = 256 producer tasks.
                for mi, ki4 in T.Parallel(BLOCK_M, BLOCK_K // 4):
                    m = bx * BLOCK_M + mi
                    k_base = ko * BLOCK_K + ki4 * 4
                    if m < M and k_base < K:
                        for vi in T.vectorized(4):
                ...
                # Load each logical weight row along its contiguous K axis.
                # The XOR layout changes only shared-memory physical placement;
                # no explicit transpose is performed by the producer.
                for di, ki4 in T.Parallel(BLOCK_D, BLOCK_K // 4):
                    d = by * BLOCK_D + di
                    k_base = ko * BLOCK_K + ki4 * 4
                    if d < D and k_base < K:
                        if d < A:
                            for vi in T.vectorized(4):
                                weight_shared[di, ki4 * 4 + vi] = sub_angle[d, k_base + vi]
                        elif d < A + N:
                            for vi in T.vectorized(4):
                                weight_shared[di, ki4 * 4 + vi] = sub_node[d - A, k_base + vi]
                        elif d < A + N + EK:
                            for vi in T.vectorized(4):
                                weight_shared[di, ki4 * 4 + vi] = sub_edge_ik[d - A - N, k_base + vi]
                        else:
                            for vi in T.vectorized(4):
                                weight_shared[di, ki4 * 4 + vi] = sub_edge_ij[d - A - N - EK, k_base + vi]
                    else:
                        for vi in T.vectorized(4):
                            weight_shared[di, ki4 * 4 + vi] = 0
                T.sync_threads()
                T.gemm(grad_shared, weight_shared, acc, transpose_B=True)
                T.sync_threads()
...
class FusedAngleUpdateFunctionBackward(torch.autograd.Function):
    ...
    def forward(
        ...
        input_kernel = fused_angle_update_backward_inputs_v3(
            M=M,
            K=K,
            A=A,
            N=N,
